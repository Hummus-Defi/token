...
task('deploy-bribe', 'Deploy a Bribe')
  ...
  .setAction(async ({ lp, reward, rate }, { ethers, getNamedAccounts, run }) => {
    const { METIS, VOTER } = await getNamedAccounts()

    const factory = await ethers.getContractFactory('Bribe')
    const bribe = await factory.deploy(VOTER, lp, reward ?? METIS, rate, reward ? false : true)
    await bribe.deployed()

    await run('verify:verify', {
      address: bribe.address,
      constructorArguments: [VOTER, lp, reward ?? METIS, rate, reward ? false : true],
    })
  })
